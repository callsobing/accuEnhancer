...
epi_length = 0
with open("%s_dnase.training.dat" % args.data_prefix, "r") as f:
    chroms_tr = []
    start_tr = []
    stop_tr = []
    label_tr = []
    ...
    for i,l in enumerate(f):
        ...
        l = l.split("\t")
        epi_length = len(l[5:])
        chroms_tr.append(l[0])
        start_tr.append(l[1])
        stop_tr.append(l[2])
        label_tr.append(int(l[3]))
        epi_mark_tr.append([float(m) for m in l[5:]])
...
label_bin_tr = np.zeros((len(label_tr), 1),  dtype=np.int)
for j in range(len(label_tr)):
    label_bin_tr[j,] = label_bin_tr[j,] + label_tr[j]
print("\n###-- Epigenetic Feature Length = %d --###" % epi_length)
epi_bin_tr = np.zeros((len(epi_mark_tr), epi_length),  dtype=np.float)
for j in range(len(epi_mark_tr)):
    epi_bin_tr[j,] = epi_mark_tr[j]
# Sample Test/ Validation and Training set according to selected mode -----------
training_rows = np.array(range(len(chroms_tr)))  # make an array of input rows to sample from once


# Process testing data (k562 sample)
with open("%s_dnase.validation.dat" % args.data_prefix, "r") as f:
    chroms_te = []
    start_te = []
    stop_te = []
    label_te = []
...
print("\nStoring Coordinates ...")
write_train_coords = open("%s_dnase_dataset_train_coords.bed" % args.data_prefix, "w")
for tr in training_rows:
    write_train_coords.write("%s\t%s\t%s\n" % (chroms_tr[tr], start_tr[tr], stop_tr[tr]))
write_test_coords = open("%s_dnase_dataset_validate_coords.bed" % args.data_prefix, "w")
for tr in test_rows:
...
train_h5f.create_dataset('epi_feature_count', data=epi_length)
train_h5f.create_dataset('training_size', data=training_rows.shape[0])

set_train_seq = train_h5f.create_dataset('training_seqs', (training_rows.shape[0], temp_seq.shape[0], temp_seq.shape[1]) , dtype='i')
train_h5f.create_dataset('training_labels', data=label_bin_tr[training_rows,])
train_h5f.create_dataset('training_epi_marks', data=epi_bin_tr[training_rows,])
